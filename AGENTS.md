...
## Съёмка кадра при проверке

Снимок на этой машине идёт около двух минут: рендер софтверный. Цикл «снял, чуть поправил,
снял снова» съедает больше времени, чем сама работа, поэтому снимать надо **только в
ключевых точках**: когда сцена вообще собралась и когда работа закончена. Промежуточную
подгонку веди расчётом (углы, высоты, попадает ли луч, влезает ли в кадр), а вызовы
отрисовки и треугольники печатает сама страница в блоке «Замер».

Снимок нужен там, где числа бессильны: свет, читаемость композиции, пересечения. Всё
остальное дешевле проверить арифметикой и кодом.
...
* `docs/scene-register.md` правило непохожести и занятые комбинации. Новая сцена обязана отличаться от каждой прежней минимум по четырём структурным осям из восьми, иначе это перекраска.
