...
const NAME_CENTER_RATIO = 0.56;
const NAME_MAX_UNITS = 16;
const NAME_TRACKING = 0.03;
// Двоение имени это разъезд каналов, а не подсветка: красный уходит влево, синий вправо.
// Фиолетовый `trip` в этой паре давал розовый неон, которого в палитре зала нет.
...
function paintScan(ctx, frame, random) {
  const step = frame.height / SCAN_BANDS;
  for (let band = 0; band < SCAN_BANDS; band += 1) {
    ctx.fillStyle = rgba(band % 2 ? PALETTE.iron : PALETTE.concrete, between(random, SCAN_ALPHA[0], SCAN_ALPHA[1]));
...
function paintNoise(ctx, frame, random, name) {
...
function microSlab(ctx, frame, text, { x, y, ink, slab }) {
...
export default {
  ...
  paint({ ctx, frame, random, event, artist, logo }) {
    ...
    fillTracked(ctx, artist.name, { ...line, x: line.x - frame.unit * GHOST_OFFSET_UNITS, color: PALETTE.blood });
    fillTracked(ctx, artist.name, { ...line, x: line.x + frame.unit * GHOST_OFFSET_UNITS, color: PALETTE.moon });
    ctx.restore();
    fillTracked(ctx, artist.name, { ...line, color: PALETTE.flame });
    ...
    microSlab(ctx, frame, artist.number, { x: frame.left, y: head, ink: PALETTE.flame, slab: PALETTE.blood });
    microSlab(ctx, frame, event.dateLabel, { x: frame.left, y: baseline + frame.unit * MICRO_LEAD_UNITS * 2, ink: PALETTE.void, slab: PALETTE.ember });
